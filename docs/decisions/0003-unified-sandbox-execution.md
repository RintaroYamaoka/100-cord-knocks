...
### 効かなかった案: vCPU を 2 に増やす (2026-09-11 実測、採用しない)

コンパイルが並列化で速くなるかを `python3 scripts/measure-sandbox-latency.py 2` で測った
(第 1 引数が vCPU 数)。温まった後の実行時間 (= コンパイル + 実行そのもの) の比較:

| 言語 | 1 vCPU | 2 vCPU |
|---|---|---|
| python | 0.62s | 0.57s |
| javascript | 0.69s | 0.69s |
| cpp | 0.80s | **0.94s** |
| java | 1.37s | 1.20s |
| csharp | 1.43s | **1.68s** |
| rust | 0.92s | 0.78s |

**差はノイズの範囲で、遅くなる言語もある。** 提出は 1 ファイルのコンパイルなので
並列化の余地がほとんど無い。1 vCPU = メモリ 2GB なので、2 にすると provisioned memory が
倍になる。**効果が無いので `VCPUS` は 1 のまま。**同じことを再び試さないようにここに残す。

### 残っている内訳 (L1 の後、2026-09-11 実測)

| 区間 | 秒 | どこで削れるか |
|---|---|---|
| サンドボックス作成 | 0.78〜0.86 | ② (使う前に作っておく / 再利用) |
| 初回実行のウォームアップ | +0.6〜4.5 (言語による) | ② / ③ (冷えた page cache のぶん。2 回目以降は 0.6〜1.7s) |
| コンパイル + 実行そのもの | 0.6〜1.7 | ここが下限。1 vCPU / 2 vCPU で変わらない |
| 停止待ち | **0 (L1 で外した)** | — |

**次にやるなら ② (再利用)**。ここから先は隔離の根拠が変わるので、この ADR とは別に決める。
なお「使う前にサンドボックスを作っておくだけ (再利用はしない)」なら
1 提出 = 使い捨ての microVM は保てるが、**使われないまま `timeout` で消えるぶんだけ
Hobby の作成枠 (5,000 回/月) を余計に食う**。枠切れは「次サイクルまで実行停止」なので、
これも体感と可用性のトレードオフとして決めてから入れること。
